...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_index:
        ...
        welcome_message = (
            f"{user_data['name']}, welcome back to TaniGPT! "
...
async def get_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"{name}, badhiya choice! {get_emoji('welcome')} "
...
async def get_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_message = (
        f"{context.user_data['name']}, signup ho gaya, swagat hai TaniGPT mein! "
...
async def clear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with open(user_file, 'w') as f:
        json.dump(user_data, f, indent=4)
    await update.message.reply_text(
        f"{user_data['name']}, tera history clear ho gaya! {get_emoji('success')}"
    )
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        personalized_response = f"{user_name}, {response} {emoji}"
        await update.message.reply_text(personalized_response)

    except Exception as e:
        logger.error(f"Error in text processing: {str(e)}")
        emoji = get_emoji("error")
        await update.message.reply_text(f"{user_name}, kuch galat ho gaya: {str(e)} {emoji}")
